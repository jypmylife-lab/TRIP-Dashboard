    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        ...
        <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
          {total > 0 && <span className={`badge ${pct === 100 ? "" : "badge-gray"}`} style={{ background: pct === 100 ? "var(--coral)" : "rgba(0,0,0,0.06)", color: "#1a1a1a" }}>{done}/{total} 완료</span>}
        </div>
      </div>
      ...
      {total > 0 && (
        <div style={{ background: "rgba(0,0,0,0.06)", borderRadius: 99, height: 12, overflow: "hidden" }}>
          <div style={{ height: "100%", width: `${pct}%`, background: "var(--coral)", borderRadius: 99, transition: "width 0.4s ease" }} />
        </div>
      )}
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
            <button type="button"
              ...
              style={{
                ...
                background: selectedAssignees.length === nicknames.length ? "var(--coral)" : "transparent",
                color: selectedAssignees.length === nicknames.length ? "#1a1a1a" : "var(--text-secondary)",
                borderColor: selectedAssignees.length === nicknames.length ? "var(--coral)" : "rgba(0,0,0,0.1)",
              }}>
              ✨ 모두
            </button>
            {nicknames.map(name => (
              <button key={name} type="button"
                ...
                style={{
                  padding: "6px 14px", borderRadius: 99, fontSize: "0.8rem", fontWeight: 800,
                  border: "2px solid", cursor: "pointer", transition: "all 0.15s",
                  background: selectedAssignees.includes(name) ? "var(--coral)" : "transparent",
                  color: selectedAssignees.includes(name) ? "#1a1a1a" : "var(--text-secondary)",
                  borderColor: selectedAssignees.includes(name) ? "var(--coral)" : "rgba(0,0,0,0.1)",
                }}>
                👤 {name}
              </button>
            ))}
